...
| `/t/{code}` and `/v/{code}` | Persistent alias lookup followed by an active same-organization session with `video_requester` | Aliases are locators, not authorization; tablet and recording details render only after the organization and role checks, and responses are non-cacheable |
