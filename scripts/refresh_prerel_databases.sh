...
STAGING_INSTANCE="r2c-prerel"
STAGING_SERVICE="r2c-tracker-prerel"
STAGING_BUCKET="r2c-tracker-prerel-flightlogs"
FIREWALL_RULE="allow-iap-postgres-r2c-prerel"
PROD_PORT="15433"
STAGE_PORT="15434"
...
if [ "${PROJECT}" != "r2c-tracker-pilot" ]; then
  echo "Refusing prerel refresh in unexpected project ${PROJECT}." >&2
  exit 1
fi
...
if pilot_gcloud compute firewall-rules describe "${FIREWALL_RULE}" \
  ...
  echo "Removing stale temporary prerel firewall rule before refresh."
  pilot_gcloud compute firewall-rules delete "${FIREWALL_RULE}" \
    --project "${DB_PROJECT}"
fi
pilot_gcloud compute firewall-rules create "${FIREWALL_RULE}" \
  ...
  --description "Temporary authenticated IAP PostgreSQL access for isolated prerel soak clone"
created_firewall="1"
...
stage_tracker_url="$(pilot_gcloud secrets versions access latest --project "${PROJECT}" --secret r2c-prerel-tracker-database-url)"
stage_control_url="$(pilot_gcloud secrets versions access latest --project "${PROJECT}" --secret r2c-prerel-control-plane-database-url)"
...
while [ "${attempt}" -lt 30 ]; do
  if "${PG_ISREADY}" -h 127.0.0.1 -p "${PROD_PORT}" -d r2c_pilot_tracker -U r2c_pilot_app >/dev/null 2>&1 \
      && "${PG_ISREADY}" -h 127.0.0.1 -p "${STAGE_PORT}" -d r2c_prerel_tracker -U r2c_prerel_tracker_user >/dev/null 2>&1; then
    ready="1"
    break
  fi
  attempt=$((attempt + 1))
...
if [ "${tracker_dump_status}" -ne 0 ] || [ "${control_dump_status}" -ne 0 ]; then
  ...
  echo "Prerel database dump failed: tracker=${tracker_dump_status} control=${control_dump_status}." >&2
  exit 1
fi

pilot_gcloud sql databases delete r2c_prerel_tracker \
  --instance "${STAGING_INSTANCE}" --project "${PROJECT}"
pilot_gcloud sql databases delete r2c_prerel_control_plane \
  --instance "${STAGING_INSTANCE}" --project "${PROJECT}"
pilot_gcloud sql databases create r2c_prerel_tracker \
  --instance "${STAGING_INSTANCE}" --project "${PROJECT}"
pilot_gcloud sql databases create r2c_prerel_control_plane \
  --instance "${STAGING_INSTANCE}" --project "${PROJECT}"

echo "Restoring independent prerel databases in parallel."
"${PG_RESTORE}" --no-owner --no-acl --exit-on-error \
  --host 127.0.0.1 --port "${STAGE_PORT}" --username r2c_prerel_tracker_user \
  --dbname r2c_prerel_tracker "${temporary_dir}/tracker.dump" \
  >"${temporary_dir}/tracker-restore.log" 2>&1 &
tracker_restore_pid="$!"
tracker_job_pid="${tracker_restore_pid}"
"${PG_RESTORE}" --no-owner --no-acl --exit-on-error \
  --host 127.0.0.1 --port "${STAGE_PORT}" --username r2c_prerel_control_user \
  --dbname r2c_prerel_control_plane "${temporary_dir}/control.dump" \
...
if [ "${tracker_restore_status}" -ne 0 ] || [ "${control_restore_status}" -ne 0 ]; then
  ...
  echo "Prerel database restore failed: tracker=${tracker_restore_status} control=${control_restore_status}." >&2
  exit 1
fi

echo "Prerel database clones refreshed in isolated Cloud SQL."
